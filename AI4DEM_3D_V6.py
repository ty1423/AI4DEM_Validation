...
x_grid = np.zeros((1, 1, grid_shape[0], grid_shape[1], grid_shape[2]))
y_grid = np.zeros((1, 1, grid_shape[0], grid_shape[1], grid_shape[2]))
z_grid = np.zeros((1, 1, grid_shape[0], grid_shape[1], grid_shape[2]))

vx_grid = np.zeros((1, 1, grid_shape[0], grid_shape[1], grid_shape[2]))
vy_grid = np.zeros((1, 1, grid_shape[0], grid_shape[1], grid_shape[2]))
vz_grid = np.zeros((1, 1, grid_shape[0], grid_shape[1], grid_shape[2]))
mask = np.zeros((1, 1, grid_shape[0], grid_shape[1], grid_shape[2]))
'''
...
x_grid[0, 0, 18, 18, 10] = 10 * cell_size
y_grid[0, 0, 18, 18, 10] = 18 * cell_size
z_grid[0, 0, 18, 18, 10] = 18 * cell_size

vx_grid[0, 0, 18, 18, 10] = 20
vy_grid[0, 0, 18, 18, 10] = 20
vz_grid[0, 0, 18, 18, 10] = 20
...
class AI4DEM(nn.Module):
    ...
    def forward(self, x_grid, y_grid,z_grid, vx_grid, vy_grid, vz_grid, fx_grid_collision, fy_grid_collision, fz_grid_collision, fx_grid_damping, fy_grid_damping, fz_grid_damping, mask, d, kn, damping_coefficient_Eta, diffx, diffy, diffz, diffvx, diffvy, diffvz, dt, input_shape, filter_size):
        ...
        for i in range(filter_size):
            for j in range(filter_size):
                for k in range(filter_size):
                    ...
                    dist = torch.sqrt(diffx**2 + diffy**2 + diffy**2)  
                    ...
                    # calculate the velocity difference between the two particles
        ...
        vx_grid = vx_grid  + (dt / particle_mass) * ( - 0 * particle_mass) * mask + (dt / particle_mass) * ( - fx_grid_boundary_right    + fx_grid_boundary_left    - fx_grid_collision - fx_grid_damping - fx_grid_left_damping    - fx_grid_right_damping   ) * mask
        vy_grid = vy_grid  + (dt / particle_mass) * ( - 0 * particle_mass) * mask + (dt / particle_mass) * ( - fy_grid_boundary_top      + fy_grid_boundary_bottom  - fy_grid_collision - fy_grid_damping - fy_grid_bottom_damping  - fy_grid_top_damping     ) * mask 
        vz_grid = vz_grid  + (dt / particle_mass) * ( - 0 * particle_mass) * mask + (dt / particle_mass) * ( - fz_grid_boundary_backward + fz_grid_boundary_forward - fz_grid_collision - fz_grid_damping - fz_grid_forward_damping - fz_grid_backward_damping) * mask 
        
        print(vy_grid[vy_grid!=0])
        print(vz_grid[vz_grid!=0])
        
        print(fy_grid_boundary_top[fy_grid_boundary_top!=0])
        print(fy_grid_boundary_bottom[fy_grid_boundary_bottom!=0])
        print(fy_grid_top_damping[fy_grid_top_damping!=0])
        print(fy_grid_bottom_damping[fy_grid_bottom_damping!=0])
        print(fz_grid_boundary_backward[fz_grid_boundary_backward!=0])
        print(fz_grid_boundary_forward[fz_grid_boundary_forward!=0])
        print(fz_grid_backward_damping[fz_grid_backward_damping!=0])
        print(fz_grid_forward_damping[fz_grid_forward_damping!=0])
...
with torch.no_grad():
  ...
  #           sc = ax.scatter(
  #               xp, yp, zp, c=vy_grid[vy_grid != 0].cpu(), cmap="turbo", s=S_graph, vmin=-20, vmax=20
  #          )
            sc = ax.scatter(yp, zp, c=vy_grid[vy_grid != 0].cpu(), cmap="turbo", s=S_graph, vmin=-40, vmax=40)
