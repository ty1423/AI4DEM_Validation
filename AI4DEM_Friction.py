        ...
        for i in range(filter_size):
            for j in range(filter_size):
                for k in range(filter_size):
                    ...
                    fx_grid_friction = torch.where(torch.lt(dist,2*d), torch.abs(friction_coefficient * fy_grid_collision + friction_coefficient * fz_grid_collision) * diffvx / torch.maximum(eplis, torch.abs(diffvx)), zeros)
                    fy_grid_friction = torch.where(torch.lt(dist,2*d), torch.abs(friction_coefficient * fx_grid_collision + friction_coefficient * fz_grid_collision) * diffvy / torch.maximum(eplis, torch.abs(diffvy)), zeros)
                    fz_grid_friction = torch.where(torch.lt(dist,2*d), torch.abs(friction_coefficient * fx_grid_collision + friction_coefficient * fy_grid_collision) * diffvy / torch.maximum(eplis, torch.abs(diffvz)), zeros)
