...
x_grid[0, 0, 10, 10] = 10 * cell_size
y_grid[0, 0, 10, 10] = 10 * cell_size
vx_grid[0, 0, 10, 10] = 5
vy_grid[0, 0, 10, 10] = 0
...
with torch.no_grad():
    for itime in range(1, ntime + 1):
        ...
        if itime % 200 == 0:
            ...
            plt.scatter(xp, yp, c=vy_grid[vy_grid!=0].cpu(), cmap='turbo', s=S_graph, vmin=-20, vmax=20)            
            cbar = plt.colorbar()
            cbar.set_label('$V_{p}$')
            ax = plt.gca()
            ...
            # Save visualization
            if itime < 10:
                save_name = "damping/"+str(itime)+".jpg"
