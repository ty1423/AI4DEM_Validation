...
for i in range(1, half_domain_size-1):
    x_grid[0, 0, domain_size-cell_size, i*2] = i * cell_size *2
    y_grid[0, 0, domain_size-cell_size, i*2] = (domain_size-cell_size) * cell_size
    vx_grid[0, 0, domain_size-cell_size, i*2] = random.uniform(-1.0,1.0)
    mask[0, 0, domain_size-cell_size, i*2] = 1
''''''
...
with torch.no_grad():
    for itime in range(1, ntime + 1):
        ...
        if itime % 200 == 0:
            ...
            plt.scatter(xp, yp, c=vy_grid[vy_grid!=0].cpu(), cmap='turbo', s=S_graph, vmin=-50, vmax=50)            
            cbar = plt.colorbar()
            cbar.set_label('$V_{p}$')
            ax = plt.gca()
