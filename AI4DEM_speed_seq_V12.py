...
class AI4DEM(nn.Module):
    ...
    def forward(self, x_grid, y_grid, vx_grid, vy_grid, fx_grid, fy_grid, mask, d, kn, diffx, diffy, dt, input_shape, filter_size):
        ...
        is_bottom_overlap = torch.gt(y_grid, 0.01) & torch.lt(y_grid, 10.1) # Overlap with bottom wall
        is_top_overlap = torch.gt(y_grid,-10+domain_size ) & torch.lt(y_grid,domain_size) # Overlap with bottom wall      
        is_left_overlap = torch.gt(x_grid, 0.01) & torch.lt(x_grid, 5.1) # Overlap with bottom wall
        is_right_overlap = torch.gt(x_grid,-10+domain_size ) & torch.lt(x_grid,domain_size) # Overlap with bottom wall
        ...
        fy_grid_boundary_bottom = kn * torch.where(is_bottom_overlap, torch.tensor(1.0, device=device), torch.tensor(0.0, device=device))*mask
        fy_grid_boundary_top = -kn * torch.where(is_top_overlap, torch.tensor(1.0, device=device), torch.tensor(0.0, device=device))*mask
        fx_grid_boundary_left = kn * torch.where(is_left_overlap, torch.tensor(1.0, device=device), torch.tensor(0.0, device=device))*mask
        fx_grid_boundary_right = -kn * torch.where(is_right_overlap, torch.tensor(1.0, device=device), torch.tensor(0.0, device=device))*mask
        vx_grid = vx_grid   - (dt / particle_mass)* ( fx_grid_boundary_left + fx_grid_boundary_right+fx_grid) 
        vy_grid = vy_grid  - (dt / particle_mass) * ( 9.8 * particle_mass) * mask  + (dt / particle_mass)* ( fy_grid_boundary_top + fy_grid_boundary_bottom+fy_grid) 
...
with torch.no_grad():
    for itime in range(1, ntime + 1):
        ...
        if itime % 10 == 0:
            ...
            # Save visualization
            save_name = f"Force_model/{itime}.jpg"
