...
xp_s = np.zeros((half_domain_size,half_domain_size))
yp_s = np.zeros((half_domain_size,half_domain_size))
...
for i in range(1, half_domain_size-1):
    for j in range(1, half_domain_size-1):
        x_grid[0, 0, j*2, i*2] = i * cell_size *2
        y_grid[0, 0, j*2, i*2] = j * cell_size *2
        mask[0, 0, j*2, i*2] = 1
...
class AI4DEM(nn.Module):
    ...
    def forward(self, x_grid, y_grid, vx_grid, vy_grid, fx_grid, fy_grid, mask, d, kn, diffx, diffy, dt, input_shape, filter_size):
        ...
        is_top_overlap = torch.gt(y_grid,domain_size-d ) # Overlap with bottom wall      
        is_left_overlap = torch.gt(x_grid, 0.01) & torch.lt(x_grid, d) # Overlap with bottom wall
        is_right_overlap = torch.gt(x_grid,domain_size-d)# Overlap with bottom wall
        ...
        fy_grid_boundary_top = kn * torch.where(is_top_overlap, torch.tensor(1.0, device=device), torch.tensor(0.0, device=device)) * mask * (y_grid - domain_size + d)
        ...
        fx_grid_boundary_right = kn * torch.where(is_right_overlap, torch.tensor(1.0, device=device), torch.tensor(0.0, device=device)) * mask * (x_grid - domain_size + d)
...
with torch.no_grad():
    for itime in range(1, ntime + 1):
        ...
        if itime % 100 == 0:
            ...
            plt.scatter(xp, yp, c=vy_grid[vy_grid!=0].cpu(), cmap='turbo', s=S_graph, vmin=-5, vmax=5)            
            cbar = plt.colorbar()
            cbar.set_label('$V_{p}$')
            ax = plt.gca()
            ...
            # Save visualization
            if itime < 10:
                save_name = "rotation/"+str(itime)+".jpg"
